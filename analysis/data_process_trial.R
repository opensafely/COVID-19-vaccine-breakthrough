...
read_csv(here::here("output", "data", "input_trial.csv"),
         n_max = 0,
         col_types = cols()) %>%
  names() %>%
  print()
...
data_extract0 <- read_csv(
  here::here("output", "data", "input_trial.csv"),
  col_types = cols_only(
...
data_processed <- data_extract %>%
  ...
  select(patient_id, 
         ...
         covid_vax_1_date, covid_vax_2_date, covid_vax_3_date, follow_up_time_vax1, follow_up_time_vax2, tbv,
...
data_processed_final <- data_processed %>%
  ...
  select(-covid_positive_test_within_2_weeks_post_vax2, 
         ...
         -covid_positive_test_date, -covid_hospital_admission_date, -death_date, -censor_date, -covid_vax_3_date) %>% 
  droplevels() 


# Save dataset as .rds files ----
write_rds(data_processed, here::here("output", "data", "data_all_trial.rds"), compress = "gz")
write_rds(data_processed_final, here::here("output", "data", "data_processed_trial.rds"), compress = "gz")
