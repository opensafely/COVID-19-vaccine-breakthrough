...
# This script 
# - produces multiple "table 2s" with the number of patients fully vaccinated (2 doses + 2 weeks) in 
#   selected clinical and demographic groups, broken by age groups
...
fs::dir_create(here::here("output", "tables", "age-breakdown"))
...
# Table 2 for each age group: <=40, 41-50, 51-64, and 80+ ----

age_groups <- unique(data_processed$ageband)
...
for (i in 1:length(age_groups)) {
  ...
  variables <- c("all", "sex", "bmi", "smoking_status", "ethnicity", "imd", "region" ,
                 ...
                 "transplant", "time_since_fully_vaccinated", "time_between_vaccinations", "prior_covid_cat")
  
  ## Counts
  counts0 <- data_processed_agegroup %>%
  ...
  counts <- counts0$table_body %>%
    ...
    filter(!(group == "prior_covid_cat" & variable == "Unknown"),
           !(group == "chronic_kidney_disease" & variable == "No CKD"),
  ...
  test_counts <- data_processed_agegroup %>%
    mutate(time_since_fully_vaccinated = cut(follow_up_time_vax2 - 14,
                                             breaks = c(0, 28, 56, 84, Inf),
                                             labels = c("0-4 weeks", "4-8 weeks", "8-12 weeks", "12+ weeks"),
                                             right = FALSE),
  ...
  ## Follow-up time
  follow_up <- data_processed_agegroup %>%
    mutate(time_since_fully_vaccinated = cut(follow_up_time_vax2 - 14,
                                             breaks = c(0, 28, 56, 84, Inf),
                                             labels = c("0-4 weeks", "4-8 weeks", "8-12 weeks", "12+ weeks"),
                                             right = FALSE),
  ...
  table2_redacted <- table2_redacted %>%
    mutate(`Fully vaccinated` = plyr::round_any(as.numeric(`Fully vaccinated`), 5),
           `Positive COVID test` = plyr::round_any(as.numeric(`Positive COVID test`), 5),
           `Hospitalised with COVID` = plyr::round_any(as.numeric(`Hospitalised with COVID`), 5),
           `Critical care with COVID` = plyr::round_any(as.numeric(`Critical care with COVID`), 5),
           `COVID Deaths` = plyr::round_any(as.numeric(`COVID Deaths`), 5))
  ...
  # Save as csv ----
  write.csv(table2_redacted, 
            here::here(paste("output/tables/age-breakdown/table2_ageband_", age_groups[i], "_redacted.csv", sep = "")))
