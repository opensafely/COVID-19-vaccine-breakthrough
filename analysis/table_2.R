...
# Table 2 shell ----
results.table <- data.frame(matrix(nrow = 26, ncol = 5))
...
results.table[6:14,3] <- (data %>% group_by(region) %>% tally())$n
...
results.table[6:14,4] <- (data %>% group_by(region) %>% tally())$n
...
results.table[6:14,5] <- (data %>% group_by(region) %>% tally())$n
